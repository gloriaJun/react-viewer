...
import { allowedPageNumber, columnGap, columnWidth, containerHeight, isScroll } from './utils/EpubSettingUtil';
import ow from 'ow';
  ...
  private calculate = async (): Promise<EpubCalculationState> => {
    return measure(() => {
      ...
      if (isScroll(this.settingState)) {
        ...
        calculation.pageUnit = calculation.pageCalculationUnit = getClientHeight();
        getSpineTotalAndTotalPage = spine => {
          const total = spine.scrollHeight;
          const totalPage = Math.floor(total / calculation.pageCalculationUnit);
          return { total, totalPage };
        };
        getTotalAndTotalPage = ({ offset, total }) => ({
          total: offset + total,
          totalPage: Math.floor((offset + total) / calculation.pageCalculationUnit),
        });
      } else {
        ...
        calculation.pageCalculationUnit = containerHeight(this.settingState);
        getSpineTotalAndTotalPage = spine => {
          const total = spine.getBoundingClientRect().height;
          const totalPage = Math.ceil(total / calculation.pageCalculationUnit);
          return { total, totalPage };
        };
